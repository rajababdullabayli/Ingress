...
public class Main {
    static void main(String[] args) {
        ...
        double[] arrayPaymentVat = new double[totalPayment];
        ...
        for (int i = 0; i < totalPayment; i++) {
            ...
            scanner.nextLine();
            System.out.println("Enter the Payment Vat: ");
            double paymentVat = scanner.nextDouble();
            arrayPaymentVat[i] = paymentVat;
        }
        for (int i = 0; i < totalPayment; i++) {
            Payment payment = new Payment();
            payment.setPaymentId(arrayPaymentId[i]);
            payment.setAmount(arrayPaymentAmount[i]);
            payment.setVat(arrayPaymentVat[i]);
